...
def researcher_system_prompt() -> str:
    return (
        ...
        "- Your first assistant message must be a search_arxiv tool call.\n"
        "- If the blackboard has no evidence, call search_arxiv; do not conclude insufficient_evidence immediately.\n"
        "- Do not submit research notes before at least one search_arxiv tool result is observed.\n"
        "- Search first, then fetch_pdf before extract_sections for selected papers when possible.\n"
        "- References must come only from tool results. Never invent paper titles, arXiv IDs, authors, years, benchmarks, or conclusions.\n\n"
...
def critic_system_prompt() -> str:
    return (
        ...
        "Check every cited paper claim against the evidence snippets included in the notes and tool_trace.\n"
        "If a paper title, arXiv ID, author, year, or claim is not present in tool results, mark it as unsupported or fake.\n"
        "You must not approve notes with evidence_count=0 and non-empty references.\n"
        "For fake citation tests, explicitly mention unsupported citation, fake citation, or citation not in evidence.\n"
